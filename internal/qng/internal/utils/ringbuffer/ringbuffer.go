...
func (r *RingBuffer[T]) PopFront() T {
	if r.Empty() {
		panic("github.com/tmc/go-iroh/internal/qng/internal/utils/ringbuffer: pop from an empty queue")
	}
...
func (r *RingBuffer[T]) PeekFront() T {
	if r.Empty() {
		panic("github.com/tmc/go-iroh/internal/qng/internal/utils/ringbuffer: peek from an empty queue")
	}
