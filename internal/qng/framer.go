...
import (
	...
	"github.com/tmc/go-iroh/internal/qng/internal/utils/minheap"
...
func (f *framer) HasData() bool {
	...
	// hasMaxDataFrame is a go-iroh addition: MAX_DATA is coalesced into the
	// framer rather than queued as an ordinary control frame.
	hasControlFrames := f.hasMaxDataFrame || len(f.streamsWithControlFrames) > 0 || len(f.controlFrames) > 0 || len(f.pathResponses) > 0
...
retransmissions:
	...
	for urgency := range f.incrementalStreams {
		...
		for range numActiveStreams {
			...
			if sf.Frame != nil {
				// go-iroh addition: hold the first STREAM frame back in
				// singleFrame so a packet carrying exactly one frame, the
				// common case, does not allocate the slice. A retransmission
				// above already appended, so len(streamFrames) != 0 skips this.
