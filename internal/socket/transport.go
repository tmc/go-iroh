...
//
// ReadFrom is not safe for concurrent use: a GRO or relay batch holds several
// datagrams, and successive calls hand them out one at a time from a cursor.
// Only the quic-go Transport reads from a MagicConn, from its single listen
// goroutine.
func (m *MagicConn) ReadFrom(p []byte) (int, net.Addr, error) {
	for {
		...
		case b := <-m.recvCh:
			...
			if len(b.data) == 0 {
				// Nothing to hand out, and an empty cursor would be
				// overwritten unreleased by the next batch.
				b.release()
				continue
			}
